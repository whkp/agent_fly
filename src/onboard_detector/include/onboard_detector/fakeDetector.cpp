...
namespace onboardDetector{
	...
	void fakeDetector::stateCB(const gazebo_msgs::ModelStatesConstPtr& allStates){
		...
		for (int i=0; i<int(this->targetIndex_.size()); ++i){
			...
			// 尝试从名称中解析尺寸，格式: prefix_XXX_YYY_ZZZ
			// 如果名称不包含尺寸信息，使用默认值
			bool sizeFound = false;
			
			// 检查名称是否包含下划线分隔的数字
			size_t lastUnderscore = name.find_last_of('_');
			if (lastUnderscore != std::string::npos && name.size() >= 11) {
				try {
					int xsizeStartIdx = name.size() - 1 - 1 - 3 * 3;
					if (xsizeStartIdx >= 0) {
						std::string xsizeStr = name.substr(xsizeStartIdx, 3);
						std::string ysizeStr = name.substr(name.size() - 1 - 3 * 2, 3);
						std::string zsizeStr = name.substr(name.size() - 3, 3);
						
						xsize = std::stod(xsizeStr) / 100.0;  // 转换为米
			...
			if (!sizeFound) {
				...
				if (name.find("person") != std::string::npos) {
					...
					ysize = 0.5;
					zsize = 1.8;
				} else if (name.find("obstacle") != std::string::npos) {
					xsize = 1.0;
					ysize = 1.0;
					zsize = 1.0;
				} else {
				...
				// 只在第一次遇到该名称时打印警告
				static std::set<std::string> warnedNames;
				if (warnedNames.find(name) == warnedNames.end()) {
					std::cout << "[Fake Detector]: Using default size for '" << name 
